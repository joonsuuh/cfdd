	...
	void solve(double t0, double t_end) {
		...
		while (t < t_end) {
			if (t >= output_dt * n) {
				...
				// vy output
				// print vymax at slice b = 0.75
				int b = 0.75 * Ny;
				double vymax = 0.0;
				double vy_temp;
				// double vymax = 0.0;
				// ios_base::app is used to append to the file instead of overwriting it...
				std::ofstream outfile("../data/kelvin_helmholtz/kh_output_vy.csv", std::ios_base::app);
				for (int i = 1; i < Nx - 1; i++) {
					vy_temp = std::abs(vy[i + b * Nx]);
					if (vy_temp > vymax) {
						vymax = vy_temp;
					}
					// outfile << vy[i + b * Nx] << ", ";
					if (i == Nx - 2) {
						outfile << vymax << std::endl;
						// outfile << std::endl;
					}
				}
				outfile.close();
