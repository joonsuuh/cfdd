...
int main(){
    ...
    for (int j = 0; j < Ny; j++) {
        for (int i = 0; i < Nx; i++) {
            ...
            // replace sin(kx) with random number generator to roll from -1 to 1
            vy[idx] = v_small * ((double)rand() / RAND_MAX * 2 - 1) * (exp(-(y-a)*(y-a) / sigma / sigma) + exp(-(y-b)*(y-b) / sigma / sigma));
